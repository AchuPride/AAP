  ...
  if (step === 2) {
    return (
      <div className="max-w-lg mx-auto">
        <div className="card text-center space-y-6 dark:bg-gray-950 dark:border-gray-900">
          <div className="w-16 h-16 rounded-full bg-green-50 dark:bg-emerald-950/20 flex items-center justify-center mx-auto">
            <HiCheckCircle className="w-10 h-10 text-safe dark:text-emerald-400" />
          </div>
          <div>
          ...
          <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/30 rounded-xl p-4 text-left">
            <p className="text-xs font-semibold text-amber-700 dark:text-amber-400 mb-1">⚠ Important</p>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Report an Incident</h1>
      ...
      {/* Privacy notice */}
      <div className="flex items-center gap-3 bg-safe/5 dark:bg-emerald-950/10 border border-safe/20 dark:border-emerald-900/30 rounded-xl px-4 py-3">
        <HiShieldCheck className="w-5 h-5 text-safe dark:text-emerald-400 shrink-0" />
        <p className="text-xs text-safe-DEFAULT dark:text-emerald-400 leading-relaxed">
      ...
      {/* Form */}
      <form onSubmit={submit} className="card space-y-5 dark:bg-gray-950 dark:border-gray-900">
        ...
        {/* Location */}
        <div>
          <label className="label">General Location <span className="text-gray-400 dark:text-gray-500 font-normal">(optional)</span></label>
          <input
            type="text"
            name="location"
            value={form.location}
            onChange={change}
            placeholder="e.g. Bamenda Up-Station, Yaoundé Mokolo, Douala Akwa..."
            className="input dark:bg-gray-900 dark:border-gray-800 dark:text-gray-100"
          />
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">General area only — do not enter your home address.</p>
        </div>
        ...
        <div>
          <label className="label">
            Evidence <span className="text-gray-400 dark:text-gray-500 font-normal">(optional — photos, documents, video)</span>
          </label>
          <label className="flex flex-col items-center gap-2 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-xl p-6 cursor-pointer hover:border-primary/40 dark:hover:border-indigo-500/40 transition-colors">
            ...
            <span className="text-xs text-gray-400 dark:text-gray-500">JPG, PNG, PDF, MP4 · Max 10 MB each · Up to 5 files</span>
            <input
              type="file"
              multiple
            ...
            <ul className="mt-2 space-y-1">
              {files.map((f, i) => (
                <li key={i} className="text-xs text-gray-600 dark:text-gray-400 flex items-center gap-2">
                  <HiCheckCircle className="w-3 h-3 text-safe dark:text-emerald-400" /> {f.name} ({(f.size / 1024).toFixed(1)} KB)
                </li>
              ))}
            </ul>
          )}
        ...
        <button type="submit" disabled={loading} className="btn-primary w-full py-3 rounded-xl text-base shadow-md shadow-primary/10">
          {loading ? (
